...
(function () {
  ...
  function load() {
    ...
    if (status) {
      status.hidden = false;
      status.textContent = T("Đang tải mã giảm giá…", "Loading vouchers…");
    }
    fetch("/app/discover/vouchers", {
      ...
      .then(function (data) {
        ...
        if (!list.length) {
          if (status) {
            status.hidden = false;
            status.textContent = T("Chưa có voucher. Vui lòng quay lại sau.", "No vouchers yet. Please check back later.");
          }
          return;
        }
      ...
      .catch(function () {
        ...
        if (status) {
          status.hidden = false;
          status.textContent = T("Không tải được voucher. Thử lại sau.", "Could not load vouchers. Try again later.");
        }
      });
  }
